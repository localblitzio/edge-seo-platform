...
.form-section h2 .btn{font-size:.75rem;padding:.3rem .7rem}
...
function renderStructuredFormBody(opts: {
  ...
  return [
    ...
    "</div></div></div>",
    '<div class="form-section" id="section-indexation"><h2>Indexation rules <button type="button" class="btn" data-add-to="indexation">+ Add</button></h2><div data-list-container="indexation"></div></div>',
    '<div class="form-section" id="section-canonicals"><h2>Canonical rules <button type="button" class="btn" data-add-to="canonicals">+ Add</button></h2><div data-list-container="canonicals"></div></div>',
    '<div class="form-section" id="section-schema-injections"><h2>Schema injections <button type="button" class="btn" data-add-to="schema_injections">+ Add</button></h2><div data-list-container="schema_injections"></div></div>',
    '<div class="form-section" id="section-static-redirects"><h2>Static redirects <button type="button" class="btn" data-add-to="redirects.static">+ Add</button></h2><div data-list-container="redirects.static"></div></div>',
    '<div class="form-section" id="section-meta-rewrites"><h2>Meta rewrites <button type="button" class="btn" data-add-to="meta_rewrites">+ Add</button></h2><div data-list-container="meta_rewrites"></div></div>',
    ...
    // Dynamic list-section rendering (indexation, canonicals,
    // schema_injections, redirects.static, meta_rewrites). Each section's
    // entries are read from the JSON, rendered as cards with field
    // controls, and synced back via input/click delegated listeners.
    "function escAttr(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;').replace(/'/g,'&#39;');}",
    "function getByPath(j,p){var c=j;for(var i=0;i<p.length;i++){if(c==null)return undefined;c=c[p[i]];}return c;}",
    "function setByPath(j,p,v){var c=j;for(var i=0;i<p.length-1;i++){var k=p[i],nk=p[i+1],nIsIdx=/^\\d+$/.test(String(nk));if(c[k]==null||typeof c[k]!=='object')c[k]=nIsIdx?[]:{};c=c[k];}var lk=p[p.length-1];c[/^\\d+$/.test(String(lk))?parseInt(lk,10):lk]=v;}",
    "function deleteByPath(j,p){var c=j;for(var i=0;i<p.length-1;i++){if(c==null||typeof c[p[i]]!=='object')return;c=c[p[i]];}var lk=p[p.length-1];if(Array.isArray(c)){c.splice(parseInt(lk,10),1);}else{delete c[lk];}}",
    "function pathParts(s){return s.split('.').map(function(x){return /^\\d+$/.test(x)?parseInt(x,10):x;});}",
    // Default-entry factories per list key.
    "var DEFAULTS={",
    "  indexation: function(){return {match:'^/.*',robots:'noindex,follow',additional_directives:[]};},",
    "  canonicals: function(){return {match:'^/.*',strategy:{type:'origin'},sync_og_url:true,sync_twitter_url:true,sync_jsonld_url:true};},",
    "  schema_injections: function(){return {match:'^/.*',schema_type:'Article',position:'head_append',payload:{'@context':'https://schema.org','@type':'Article'}};},",
    "  'redirects.static': function(){return {from:'/old-path',to:'/new-path',status:'301',preserve_query:true};},",
    "  meta_rewrites: function(){return {match:'^/.*',tag:'title',value:''};}",
    "};",
    // Renderers per list key — produce HTML for ONE entry card.
    ...
    '  return \'<div class="list-entry"><div class="form-grid">\'+',
    "    '<div><label>match (regex)</label><input type=\"text\" data-path=\"indexation.'+idx+'.match\" value=\"'+escAttr(e.match)+'\"></div>'+",
    "    '<div><label>robots</label><select data-path=\"indexation.'+idx+'.robots\">'+",
    "      ['index,follow','noindex,follow','noindex,nofollow','index,nofollow'].map(function(v){return '<option value=\"'+v+'\"'+(e.robots===v?' selected':'')+'>'+v+'</option>';}).join('')+",
    "    '</select></div>'+",
    '    \'<div class="full-width"><label>additional_directives</label><div class="checkbox-row">\'+',
    "      dirOpts.map(function(opt){return '<label class=\"checkbox-inline\"><input type=\"checkbox\" data-path=\"indexation.'+idx+'.additional_directives\" data-checkbox-value=\"'+escAttr(opt)+'\"'+(dirs.indexOf(opt)!==-1?' checked':'')+'> '+escAttr(opt)+'</label>';}).join('')+",
    "    '</div></div>'+",
    '  \'</div><div class="list-entry-foot"><button type="button" class="btn btn-danger" data-remove-from="indexation" data-remove-idx="\'+idx+\'">Remove</button></div></div>\';',
    "}",
    "function renderCanonicalEntry(e,idx){",
    "  var stype=(e.strategy&&e.strategy.type)||'origin';",
    "  var surl=(e.strategy&&e.strategy.url)||'';",
    '  return \'<div class="list-entry"><div class="form-grid">\'+',
    "    '<div><label>match (regex)</label><input type=\"text\" data-path=\"canonicals.'+idx+'.match\" value=\"'+escAttr(e.match)+'\"></div>'+",
    "    '<div><label>strategy.type</label><select data-path=\"canonicals.'+idx+'.strategy.type\">'+",
    "      ['self','origin','custom','noindex'].map(function(v){return '<option value=\"'+v+'\"'+(stype===v?' selected':'')+'>'+v+'</option>';}).join('')+",
    "    '</select></div>'+",
    "    (stype==='custom' ? '<div class=\"full-width\"><label>strategy.url</label><input type=\"text\" data-path=\"canonicals.'+idx+'.strategy.url\" value=\"'+escAttr(surl)+'\" placeholder=\"https://...\"></div>' : '')+",
    ...
    '  \'</div><div class="list-entry-foot"><button type="button" class="btn btn-danger" data-remove-from="canonicals" data-remove-idx="\'+idx+\'">Remove</button></div></div>\';',
    "}",
    ...
    "  var payloadStr;try{payloadStr=JSON.stringify(e.payload||{},null,2);}catch(e){payloadStr='{}';}",
    '  return \'<div class="list-entry"><div class="form-grid">\'+',
    "    '<div><label>match (regex)</label><input type=\"text\" data-path=\"schema_injections.'+idx+'.match\" value=\"'+escAttr(e.match)+'\"></div>'+",
    "    '<div><label>schema_type</label><select data-path=\"schema_injections.'+idx+'.schema_type\">'+",
    "      schemaTypes.map(function(v){return '<option value=\"'+v+'\"'+(e.schema_type===v?' selected':'')+'>'+v+'</option>';}).join('')+",
    "    '</select></div>'+",
    "    '<div><label>position</label><select data-path=\"schema_injections.'+idx+'.position\">'+",
    "      positions.map(function(v){return '<option value=\"'+v+'\"'+(e.position===v?' selected':'')+'>'+v+'</option>';}).join('')+",
    "    '</select></div>'+",
    '    \'<div class="full-width"><label>payload (JSON-LD)</label><textarea data-path="schema_injections.\'+idx+\'.payload" data-json="1" style="min-height:140px">\'+escAttr(payloadStr)+\'</textarea><div class="field-hint">Must parse as JSON. Edit the @context, @type, and properties for the schema_type above.</div></div>\'+',
    '  \'</div><div class="list-entry-foot"><button type="button" class="btn btn-danger" data-remove-from="schema_injections" data-remove-idx="\'+idx+\'">Remove</button></div></div>\';',
    "}",
    "function renderStaticRedirectEntry(e,idx){",
    '  return \'<div class="list-entry"><div class="form-grid">\'+',
    "    '<div><label>from (path)</label><input type=\"text\" data-path=\"redirects.static.'+idx+'.from\" value=\"'+escAttr(e.from)+'\"></div>'+",
    "    '<div><label>to (path or URL)</label><input type=\"text\" data-path=\"redirects.static.'+idx+'.to\" value=\"'+escAttr(e.to)+'\"></div>'+",
    "    '<div><label>status</label><select data-path=\"redirects.static.'+idx+'.status\">'+",
    "      ['301','302','307','308','410'].map(function(v){return '<option value=\"'+v+'\"'+((e.status||'301')===v?' selected':'')+'>'+v+'</option>';}).join('')+",
    "    '</select></div>'+",
    ...
    '  \'</div><div class="list-entry-foot"><button type="button" class="btn btn-danger" data-remove-from="redirects.static" data-remove-idx="\'+idx+\'">Remove</button></div></div>\';',
    "}",
    ...
    '  return \'<div class="list-entry"><div class="form-grid">\'+',
    "    '<div><label>match (regex)</label><input type=\"text\" data-path=\"meta_rewrites.'+idx+'.match\" value=\"'+escAttr(e.match)+'\"></div>'+",
    "    '<div><label>tag</label><select data-path=\"meta_rewrites.'+idx+'.tag\">'+",
    "      tags.map(function(v){return '<option value=\"'+v+'\"'+(e.tag===v?' selected':'')+'>'+v+'</option>';}).join('')+",
    "    '</select></div>'+",
    '    \'<div class="full-width"><label>value</label><input type="text" data-path="meta_rewrites.\'+idx+\'.value" value="\'+escAttr(e.value)+\'"></div>\'+',
    '  \'</div><div class="list-entry-foot"><button type="button" class="btn btn-danger" data-remove-from="meta_rewrites" data-remove-idx="\'+idx+\'">Remove</button></div></div>\';',
    "}",
    "var RENDERERS={indexation:renderIndexationEntry,canonicals:renderCanonicalEntry,schema_injections:renderSchemaInjectionEntry,'redirects.static':renderStaticRedirectEntry,meta_rewrites:renderMetaRewriteEntry};",
    "function renderListSection(key){",
    "  var container=document.querySelector('[data-list-container=\"'+key+'\"]');if(!container)return;",
    "  var json=safeParse();if(!json)return;",
    "  var arr=getByPath(json,pathParts(key));",
    "  if(!Array.isArray(arr)){arr=[];}",
    "  if(arr.length===0){container.innerHTML='<div class=\"empty\">none configured</div>';return;}",
    "  var renderer=RENDERERS[key];",
    "  container.innerHTML=arr.map(function(e,i){return renderer(e,i);}).join('');",
    "}",
    "function renderAllLists(){Object.keys(RENDERERS).forEach(renderListSection);}",
    // Delegated input handler for list-entry fields.
    "document.addEventListener('input',function(e){",
    "  var t=e.target;if(!t||!t.dataset||!t.dataset.path)return;",
    "  var json=safeParse();if(!json)return;",
    "  var p=pathParts(t.dataset.path);",
    "  var v;",
    "  if(t.dataset.checkboxValue!==undefined){",
    "    var cur=getByPath(json,p);if(!Array.isArray(cur))cur=[];",
    "    var val=t.dataset.checkboxValue;var has=cur.indexOf(val)!==-1;",
    "    if(t.checked&&!has)cur.push(val);else if(!t.checked&&has)cur.splice(cur.indexOf(val),1);",
    "    setByPath(json,p,cur);",
    "    ta.value=JSON.stringify(json,null,2);return;",
    "  }",
    "  if(t.dataset.bool==='1'){v=t.checked;}",
    "  else if(t.dataset.json==='1'){try{v=JSON.parse(t.value);t.style.borderColor='';}catch(err){t.style.borderColor='var(--red)';return;}}",
    "  else if(t.tagName==='SELECT'||t.type==='text'||t.type==='email'){v=t.value;}",
    "  else{v=t.value;}",
    "  setByPath(json,p,v);",
    "  ta.value=JSON.stringify(json,null,2);",
    "  // Re-render the affected section if a strategy.type select changed (it gates the strategy.url field).",
    "  if(t.dataset.path.indexOf('canonicals.')===0&&t.dataset.path.indexOf('.strategy.type')!==-1){renderListSection('canonicals');}",
    "});",
    // Delegated change handler for select/checkbox (covers checkboxes that don't fire 'input' on all browsers).
    "document.addEventListener('change',function(e){",
    "  var t=e.target;if(!t||!t.dataset||!t.dataset.path)return;",
    "  if(t.tagName==='SELECT'||t.type==='checkbox'){t.dispatchEvent(new Event('input',{bubbles:true}));}",
    "});",
    // Delegated click handler for add/remove buttons.
    "document.addEventListener('click',function(e){",
    "  var t=e.target;if(!t||!t.dataset)return;",
    "  if(t.dataset.addTo){",
    "    var json=safeParse();if(!json){alert('JSON has parse errors — fix them in the textarea before adding entries.');return;}",
    "    var key=t.dataset.addTo,fac=DEFAULTS[key];",
    "    var p=pathParts(key),arr=getByPath(json,p);",
    "    if(!Array.isArray(arr)){arr=[];setByPath(json,p,arr);}",
    "    arr.push(fac());",
    "    ta.value=JSON.stringify(json,null,2);",
    ...
    "    var json2=safeParse();if(!json2)return;",
    "    var key2=t.dataset.removeFrom,idx=parseInt(t.dataset.removeIdx,10);",
    "    var p2=pathParts(key2),arr2=getByPath(json2,p2);",
    "    if(Array.isArray(arr2)){arr2.splice(idx,1);ta.value=JSON.stringify(json2,null,2);renderListSection(key2);}",
    "    return;",
    "  }",
    "});",
    // Re-render lists when textarea changes (someone hand-edits the JSON).
    "var lastListSig='';",
    "function maybeRerenderLists(){",
    "  var j=safeParse();if(!j)return;",
    "  var sig=JSON.stringify({i:j.indexation,c:j.canonicals,s:j.schema_injections,r:j.redirects&&j.redirects.static,m:j.meta_rewrites});",
    "  if(sig!==lastListSig){lastListSig=sig;renderAllLists();}",
    "}",
    "ta.addEventListener('input',function(){syncFromJson();maybeRerenderLists();});",
    "syncFromJson();renderAllLists();maybeRerenderLists();",
    "})();",
    "</script>",
  ].join("");
}
